...
```
Note: This tool is a disk I/O and CPU intensive task. If it runs slowly, make sure that your system is not overloaded with other tasks. Check Activity Monitor (macOS) to see the usage. In my testing, it takes around 1m15s on a Macbook Pro with M2Pro. The max memory usage is around  1.4GB.
...
I am autogenerating the API server code using OpenAPI specification. The spec file is located at `openapi/api-1.yaml`. The tool I used is [oapi-codegen](https://github.com/oapi-codegen/oapi-codegen).
